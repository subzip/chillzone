...
import { getOneCons, updateCons } from "../../utils";

interface UpdateConsProps {
  active: boolean;
...
const UpdateCons = ({ id, active, setActive, modalType }: UpdateConsProps) => {
  ...
  const result = {
    ...
    title,
    isAvaliable,
    resistant: Number(resistant),
    price,
    imageUrl,
  };
  useEffect(() => {
    const response = getOneCons(id);
    response.then((el) => {
      ...
      setResistant(String(el.data.resistant));
